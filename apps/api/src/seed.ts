...
const pages = [
  {
    address: 'tidepool.zz', title: 'Notes from the tidepool', authorId: 'mira',
    heading: 'The ocean, at ankle height.', label: 'FIELD NOTES / 01',
    paragraphs: [
    ...
    links: [['moss.zz', 'A slower kind of garden'], ['night-train.zz', 'Take the long way home'], ['lost-lighthouse.zz', 'The lighthouse keeper’s notebook (last seen in winter)']],
  },
  {
    address: 'moss.zz', title: 'A small green atlas', authorId: 'mira',
    heading: 'Pay attention to the overlooked.', label: 'A SMALL GREEN ATLAS',
    paragraphs: [
    ...
    links: [['observatory.zz', 'From the very small to the very distant'], ['seed-library.zz', 'Borrow something that grows'], ['tidepool.zz', 'Back to the shore']],
  },
  {
    address: 'observatory.zz', title: 'The backyard observatory', authorId: 'noor',
    heading: 'A clear sky is an invitation.', label: 'THE BACKYARD OBSERVATORY',
    paragraphs: [
    ...
    links: [['radio-room.zz', 'Listen to the sky'], ['paper-plane.zz', 'Fold a little flight'], ['night-train.zz', 'Watch stars through a train window']],
  },
  {
    address: 'radio-room.zz', title: 'Signals from the spare room', authorId: 'noor',
    heading: 'There is somebody out there.', label: 'RADIO ROOM / ON AIR',
    paragraphs: [
    ...
    links: [['repair-cafe.zz', 'Things worth repairing'], ['night-train.zz', 'The midnight service'], ['silent-station.zz', 'An old station frequency']],
  },
  {
    address: 'night-train.zz', title: 'Letters from the night train', authorId: 'eli',
    heading: 'The journey is a place, too.', label: 'LETTERS IN TRANSIT',
    paragraphs: [
    ...
    links: [['bread-club.zz', 'Breakfast at the next stop'], ['paper-plane.zz', 'A letter that can fly'], ['tidepool.zz', 'Where the estuary meets the sea']],
  },
  {
    address: 'bread-club.zz', title: 'The Sunday bread club', authorId: 'eli',
    heading: 'Flour, water, time, company.', label: 'SUNDAY BREAD CLUB',
    paragraphs: [
    ...
    links: [['seed-library.zz', 'The other lending library'], ['repair-cafe.zz', 'Fix the kitchen scale'], ['night-train.zz', 'A loaf for the journey']],
  },
  {
    address: 'seed-library.zz', title: 'The neighborhood seed library', authorId: 'jules',
    heading: 'Borrow a seed. Return a story.', label: 'OPEN EVERY SATURDAY',
    paragraphs: [
    ...
    links: [['moss.zz', 'Gardens without a gardener'], ['bread-club.zz', 'Another culture to share'], ['field-recordings.zz', 'Listen to the allotment']],
  },
  {
    address: 'repair-cafe.zz', title: 'The repair café', authorId: 'jules',
    heading: 'Broken is a beginning.', label: 'SECOND SATURDAY / COMMUNITY HALL',
    paragraphs: [
    ...
    links: [['radio-room.zz', 'A receiver with another life'], ['paper-plane.zz', 'Make something from almost nothing'], ['missing-manual.zz', 'The missing service manual']],
  },
  {
    address: 'paper-plane.zz', title: 'The paper aerodrome', authorId: 'sam',
    heading: 'Ordinary paper. Brief miracles.', label: 'THE PAPER AERODROME',
    paragraphs: [
    ...
    links: [['field-recordings.zz', 'Sounds from the landing strip'], ['observatory.zz', 'Aim a little higher'], ['night-train.zz', 'Other ways to travel']],
  },
  {
    address: 'field-recordings.zz', title: 'An atlas of ordinary sounds', authorId: 'sam',
    heading: 'Close your eyes. Stay a while.', label: 'AN ATLAS OF ORDINARY SOUNDS',
    paragraphs: [
    ...
    links: [['tidepool.zz', 'The shore has a voice'], ['seed-library.zz', 'Sounds that grow'], ['radio-room.zz', 'Listening beyond the neighborhood']],
  },
];
...
async function seed() {
  ...
  try {
    for (const person of people) await db.people.updateOne({ _id: person._id }, { $setOnInsert: person }, { upsert: true });
    for (const page of pages) {
      ...
      await db.sites.updateOne({ address: page.address }, { $setOnInsert: {
        address: page.address, title: page.title, authorId: page.authorId, html,
        text: visibleText(html), publishedAt: '2026-09-01T09:00:00.000Z',
      } }, { upsert: true });
    }
    for (let personIndex = 0; personIndex < people.length; personIndex++) {
      ...
      for (let index = 0; index < count; index++) {
        ...
        await db.visits.updateOne({ _id: id }, { $setOnInsert: {
          _id: id, personId: people[personIndex]._id,
          ...
          visitedAt: new Date(Date.UTC(2026, 8, 20, 10) + index * (3_600_000 / (count - 1))).toISOString(),
        } }, { upsert: true });
      }
    }
